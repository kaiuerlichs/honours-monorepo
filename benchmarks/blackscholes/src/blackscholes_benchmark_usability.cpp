...
void run_serial() {
  ...
  for (int i = 0; i < options_count; ++i) {
    prices[i] = BlkSchlsEqEuroNoDiv(options[i].s, options[i].strike, options[i].r, options[i].v, options[i].t, options[i].OptionType, 0);
  }
...
void run_openmpi() {
  ...
  MPI_Scatterv(options.data(), sendcounts.data(), displs.data(), mpi_option_type, local_options.data(), recvcount, mpi_option_type, 0, MPI_COMM_WORLD);
#pragma omp parallel for num_threads(threads)
  for (int i = 0; i < recvcount; ++i) {
    local_prices[i] = BlkSchlsEqEuroNoDiv(local_options[i].s, local_options[i].strike, local_options[i].r, local_options[i].v, local_options[i].t, local_options[i].OptionType == 'C' ? 0 : 1, 0);
  }
  prices.resize(options_count);
  MPI_Gatherv(local_prices.data(), recvcount, MPI_FLOAT, prices.data(), sendcounts.data(), displs.data(), MPI_FLOAT, 0, MPI_COMM_WORLD);
  if (rank == 0)
    write_prices_to_file();
...
void run_hmpcore() {
  ...
  MPI_Datatype types[9] = {MPI_FLOAT, MPI_FLOAT, MPI_FLOAT,MPI_FLOAT, MPI_FLOAT, MPI_FLOAT, MPI_CHAR,  MPI_FLOAT, MPI_FLOAT};
  ...
  MPI_Type_create_struct(nitems, blocklengths, offsets, types,&mpi_option_type);
  if (cluster->on_master())
    read_options_from_file();
  auto map = std::make_unique<hmp::Map<OptionData, float>>(cluster, hmp::Distribution::CORE_COUNT);
  map->set_mpi_in_type(mpi_option_type);
  prices = map->execute(options, [](OptionData opt) {return BlkSchlsEqEuroNoDiv(opt.s, opt.strike, opt.r, opt.v, opt.t, opt.OptionType == 'C' ? 0 : 1, 0);});
  if (cluster->on_master())
    write_prices_to_file();
}
...
void run_hmpfreq() {
  ...
  prices = map->execute(options, [](OptionData opt) {return BlkSchlsEqEuroNoDiv(opt.s, opt.strike, opt.r, opt.v, opt.t,
